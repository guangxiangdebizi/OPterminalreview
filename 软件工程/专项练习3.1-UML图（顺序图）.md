...
| **创建对象** | Note标注 + 自调用 | 创建新对象 | new Object() |
| **销毁消息** | 实线+X | 销毁对象 | delete/释放 |
| **自调用** | 回指自己的箭头 ↻ | 对象调用自己的方法 | 递归、内部方法 |

**注：** Mermaid中创建对象用`Note over`标注，然后用自调用表示创建动作
...
**注：** order:Order订单对象在运行时动态创建，用Note标注
...
| **创建对象** | Note标注 + 自调用 | Note: 创建order:Order |
...
sequenceDiagram
    ...
    alt 密码正确
        Note over Auth: 创建session:Session
        Auth->>Auth: 创建会话对象
        Auth-->>UI: 登录成功
        UI-->>U: 显示登录成功
    else 密码错误
...
sequenceDiagram
    ...
    alt 资格验证通过
        ...
        alt 图书可借
            Note over BS: 创建record:BorrowRecord
            BS->>BS: 创建借阅记录
...
- 创建对象：用Note标注，只有在"资格通过且图书可借"时才创建借阅记录
